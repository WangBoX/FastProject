...
#import "WBPickerView.h"

NSString * const WBbackgroundColor = @"backgroundColor";
NSString * const WBtextColor = @"textColor";
NSString * const WBtoolbarColor = @"toolbarColor";
NSString * const WBbuttonColor = @"buttonColor";
NSString * const WBbuttonCancelTitle = @"buttonCancelTitle";
NSString * const WBbuttonDoneTitle = @"buttonDoneTitle";
NSString * const WBbuttonCancelImgName = @"buttonCancelImgName";
NSString * const WBbuttonDoneImgName = @"buttonDoneImgName";
NSString * const WBfont = @"font";
NSString * const WBvalueY = @"yValueFromTop";
NSString * const WBtoolbarBackgroundImage = @"toolbarBackgroundImage";
NSString * const WBtextAlignment = @"textAlignment";
NSString * const WBshowsSelectionIndicator = @"showsSelectionIndicator";

@interface WBPickerView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
@property (nonatomic, strong) NSString *chosenObject;
...
@implementation WBPickerView

#pragma mark - Singleton

+ (WBPickerView*)sharedView {
  static dispatch_once_t once;
  static WBPickerView *sharedView;
...
+(void)showPickerViewInView: (UIView *)view
         ...
         withSelectedObject: (NSString *)selectedObject
                withOptions: (NSDictionary *)options                 completion: (void(^)(NSString *selectedString))completion{
    
  [self sharedView].chosenObject = selectedObject;
...
+(void)showPickerViewInView:(UIView *)view
    ...
    objectToStringConverter:(NSString *(^)(id))converter                 completion:(void (^)(id))completion {
...
-(void)dismiss{
 [WBPickerView dismissWithCompletion:self.onDismissCompletion];
}
- (void)cancel{
    [UIView animateWithDuration:0.3
                     ...
                     animations:^{
                         ...
                         [_pickerContainerView setTransform:CGAffineTransformMakeTranslation(0.0, CGRectGetHeight(_pickerContainerView.frame))];
                             ...
                             [WBPickerView removePickerView];
                         self.onDismissCompletion(self.chosenObject);
                     }];
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  if (_chosenObject!=nil) {
    selectedRow = [_pickerViewArray indexOfObject:_chosenObject];
  }else{
  ...
  textAlignment = options[WBtextAlignment];
  
  _pickerViewTextAlignment = 1;
  
  if (textAlignment != nil) {
  _pickerViewTextAlignment = [options[WBtextAlignment] integerValue];
  }
  
  BOOL showSelectionIndicator = [options[WBshowsSelectionIndicator] boolValue];
  ...
  UIColor *pickerViewBackgroundColor = [[UIColor alloc] initWithCGColor:[options[WBbackgroundColor] CGColor]];
  UIColor *pickerViewTextColor = [[UIColor alloc] initWithCGColor:[options[WBtextColor] CGColor]];
  UIColor *toolbarBackgroundColor = [[UIColor alloc] initWithCGColor:[options[WBtoolbarColor] CGColor]];
  UIColor *buttonTextColor = [[UIColor alloc] initWithCGColor:[options[WBbuttonColor] CGColor]];
  UIFont *pickerViewFont = [[UIFont alloc] init];
  pickerViewFont = options[WBfont];
  _yValueFromTop = [options[WBvalueY] floatValue];
  
  [self setFrame: view.bounds];
  [self setBackgroundColor:[UIColor clearColor]];
  
  UIImage * toolbarImage = options[WBtoolbarBackgroundImage];
  
  //Whole screen with PickerView and a diWBed background
  ...
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 260.0, kSCREEN_WIDTH, 260.0)];
  ...
  if (toolbarImage!=nil) {
    ...
    if (options[WBbuttonCancelImgName] == nil) {
        cancleItem = [[UIBarButtonItem alloc] initWithTitle:options[WBbuttonCancelTitle]?options[WBbuttonCancelTitle]:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
        [cancleItem setTintColor:buttonTextColor];
    }else{
        cancleItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:options[WBbuttonCancelImgName]] style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
    }
   
    UIBarButtonItem * flexibleSpace = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    if (options[WBbuttonDoneImgName] == nil) {
        _pickerViewBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:options[WBbuttonDoneTitle]?options[WBbuttonDoneTitle]:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(dismiss)];
        [_pickerViewBarButtonItem setTintColor:buttonTextColor];
    }else{
        _pickerViewBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:options[WBbuttonDoneImgName]] style:UIBarButtonItemStylePlain target:self action:@selector(dismiss)];
    }
  _pickerViewToolBar.items = @[cancleItem, flexibleSpace, _pickerViewBarButtonItem];
