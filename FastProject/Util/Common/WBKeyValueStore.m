...
@interface WBKeyValueStore()

@property (strong, nonatomic) FMDatabaseQueue * dbQueue;

@end

@implementation WBKeyValueStore
...
- (void)createTableWithName:(NSString *)tableName {
    if ([WBKeyValueStore checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    WBKeyValueItem * item = [self getWBKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (WBKeyValueItem *)getWBKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    if (json) {
        ...
        WBKeyValueItem * item = [[WBKeyValueItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (WBKeyValueItem * item in result) {
        error = nil;
